...
import type {FormInputValueType, FormInputValueUpdateType, IFormInputFormatter, IFormInputValidator} from './types';
...
@Component({
    ...
    components: {
        VuTemplate: templateFactory(VuFormInputTemplate) as any
    },
    mixins: [ElementIdAwareMixin]
})
export default class VuFormInputAbstract extends Mixins(ElementIdAwareMixin)
{
     ...
     */
    @Prop({type: Function})
    readonly formatter!: IFormInputFormatter;
     ...
     */
    @Prop({type: Function})
    readonly validator!: IFormInputValidator;
    ...
    public get _inputFieldAttr(): PlainObject
    {
        ...
        return filter(this.$props, (_, k) => allowedAttrs.indexOf(k) !== -1);
    }
